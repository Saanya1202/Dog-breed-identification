...
def classify_images(images_dir, results_dic, model):
    ...
    for filename, label_list in results_dic.items():
        ...
        # Format: lowercase & strip spaces
        classifier_label = classifier_label.lower().strip()

        # True pet label (from results_dic index 0)
        pet_label = label_list[0]

        # Check if pet label is in classifier label string
        # (We add spaces around to ensure full word match)
        if pet_label in classifier_label:
            match = 1
        else:
            match = 0

        # Add classifier label and match result to the list
