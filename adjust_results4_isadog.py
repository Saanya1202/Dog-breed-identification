...
def adjust_results4_isadog(results_dic, dogfile):
    ...
    """           
    # Read all valid dog names from the file
    ...
    for key in results_dic:
        ...
        # Check if classifier label is a dog (can have multiple names separated by commas)
        is_classifier_dog = 0
        for name in classifier_label.lower().split(","):
            if name.strip() in dog_names:
                is_classifier_dog = 1
                break

        # Append the new information to the results list
